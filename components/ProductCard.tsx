...
export const ProductCard: React.FC<ProductCardProps> = ({ product, onClick, onAddToCart, onToggleWishlist, isWishlisted }) => {
  return (
    <div className="group bg-white border border-gray-100 p-4 transition hover:shadow-xl relative overflow-hidden">
       ...
       <div className="absolute left-4 top-4 z-10 flex flex-col space-y-2 transition-all duration-300 opacity-100 translate-x-0 lg:opacity-0 lg:-translate-x-10 lg:group-hover:opacity-100 lg:group-hover:translate-x-0">
        <button 
          ...
          className="w-10 h-10 bg-[#e31e24] text-white rounded-full flex items-center justify-center hover:bg-black transition shadow-md"
          title="Add to Cart"
        >
        ...
        <button 
          ...
          className={`w-10 h-10 rounded-full flex items-center justify-center transition shadow-md ${isWishlisted ? 'bg-black text-[#e31e24]' : 'bg-[#e31e24] text-white hover:bg-black'}`}
          title={isWishlisted ? "Remove from Wishlist" : "Add to Wishlist"}
        >
          <Heart size={16} fill={isWishlisted ? "currentColor" : "none"} />
        </button>
        <button className="w-10 h-10 bg-[#e31e24] text-white rounded-full flex items-center justify-center hover:bg-black transition shadow-md" title="Compare">
          <RefreshCw size={16} />
        </button>
      </div>
      ...
      <div className="text-center">
        <h3 
          className="text-gray-600 text-sm mb-2 h-10 overflow-hidden cursor-pointer hover:text-[#f10044] transition"
          onClick={() => onClick(product.id)}
        >
          {product.name}
        </h3>
